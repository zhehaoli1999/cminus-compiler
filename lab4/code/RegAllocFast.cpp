...
// TODO PhysReg 代表 physical register
...
bool RegAllocFast::mayLiveOut(unsigned VirtReg) {
  if (MayLiveAcrossBlocks.test(Register::virtReg2Index(VirtReg))) {
    ...
    return !MBB->succ_empty(); // 如果 succ_empty()，那么返回的是false，
  }
...
void RegAllocFast::allocateInstruction(MachineInstr &MI) {
  ...
  // early clobber 代表的是Register definition happens before uses.
  ...
  for (unsigned i = 0, e = MI.getNumOperands(); i != e; ++i) {
    ...
    // regmask是：Bitset of preserved registers, others are clobbered
    if (MO.isRegMask()) {
